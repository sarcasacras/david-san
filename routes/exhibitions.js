...
router.post('/', upload.fields([{ name: 'thumbnail', maxCount: 1 }, { name: 'images' }]), auth.requireAdmin, async (req, res) => {
    ...
    try {
        for (const file of req.files.images) {
            const result = await cloudinary.uploader.upload(file.path, { quality: 60, format: 'webp' });
            images.push({
        ...
        if (req.files.thumbnail) {
            ...
            const thumbnailResult = await cloudinary.uploader.upload(thumbnailFile.path, { quality: 60, format: 'webp' });
            thumbnail = {
